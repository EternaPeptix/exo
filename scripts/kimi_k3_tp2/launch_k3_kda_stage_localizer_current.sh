...
die() {
  echo "k3 current-stack KDA stage localizer: $*" >&2
  exit 2
}
...
required_paths=(
  K3_KDA_LOCALIZER_ROOT
  K3_TP_TOOLS_ROOT
  K3_TP_RANK0_ROOT
  K3_TP_RANK1_ROOT
  K3_TP_HOSTFILE
  K3_TP_TRANSPORT_CONTRACT
  K3_KDA_LOCALIZER_ARTIFACT_ROOT
  K3_TP_LAUNCHER
  K3_MLX_LM_ROOT
...
localizer_root="${K3_KDA_LOCALIZER_ROOT}"
...
artifact_root="${K3_KDA_LOCALIZER_ARTIFACT_ROOT}"
launcher="${K3_TP_LAUNCHER}"
mlx_lm_root="${K3_MLX_LM_ROOT}"
mlx_core_override="${K3_MLX_CORE_OVERRIDE}"

[[ -d "${localizer_root}" ]] ||
  die "K3_KDA_LOCALIZER_ROOT is not a directory"
[[ -f "${localizer_root}/k3_kda_stage_localizer.py" ]] ||
  die "K3_KDA_LOCALIZER_ROOT has no k3_kda_stage_localizer.py"
...
transport_mode="${K3_KDA_LOCALIZER_TRANSPORT_MODE:-mesh}"
file_hashes="${K3_KDA_LOCALIZER_FILE_HASHES:-0}"
exact_speculative_kda="${K3_KDA_LOCALIZER_EXACT_SPECULATIVE_KDA:-0}"
[[ "${transport_mode}" == "mesh" ]] ||
  die "K3_KDA_LOCALIZER_TRANSPORT_MODE must be mesh"
[[ "${file_hashes}" == "0" || "${file_hashes}" == "1" ]] ||
  die "K3_KDA_LOCALIZER_FILE_HASHES must be 0 or 1"
[[ "${exact_speculative_kda}" == "0" || "${exact_speculative_kda}" == "1" ]] ||
  die "K3_KDA_LOCALIZER_EXACT_SPECULATIVE_KDA must be 0 or 1"
[[ "${MLX_JACCL_RING+x}" != "x" ]] ||
...
artifact="${artifact_root}/k3-kda-stage-localizer-current-128p-w2-${stamp}-p$$-mesh.json"
arguments=(
  "${localizer_root}/k3_kda_stage_localizer.py"
...
exec "${launcher}" \
  ...
  --env "MLX_LM_KIMI_K3_EXACT_SPECULATIVE_KDA=${exact_speculative_kda}" \
  -- \
  "${arguments[@]}"
