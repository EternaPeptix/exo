...
# Exact immutable metadata inventory from SOURCE_REPO@SOURCE_REVISION.  This
# must remain byte-for-byte aligned with ``k3_tp_checkpoint.py`` because Python
# files below can execute under ``trust_remote_code=True``.
...
def _verify_metadata_contract(
    ...
    expected_metadata = PINNED_METADATA_FILES
    if normalized != expected_metadata:
        raise RankLocalLoadError(
