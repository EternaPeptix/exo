...
| `EXO_MLX_K3_VOCAB_PARALLEL_GREEDY` | Opt in to compact Kimi K3 tensor-parallel greedy sampling. With temperature zero and no processors or logprobs, ranks exchange only their local maximum and global token ID instead of full-vocabulary logits. Incompatible requests use the standard full-logit path. Must be `0` or `1`. | `0` |
